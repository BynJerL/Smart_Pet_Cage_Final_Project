...
void read_button_state(void) {
  if (pcf1.digitalRead(L_BUTTON)) {
    buttonState |= _BV(L_BUTTON);
  } else {
    buttonState &= ~_BV(L_BUTTON);
  }

  if (pcf1.digitalRead(C_BUTTON)) {
    buttonState |= _BV(C_BUTTON);
  } else {
    buttonState &= ~_BV(C_BUTTON);
  }

  if (pcf1.digitalRead(R_BUTTON)) {
    buttonState |= _BV(R_BUTTON);
  } else {
    buttonState &= ~_BV(R_BUTTON);
  }

  if (pcf1.digitalRead(GATE_BUTTON)) {
    buttonState |= _BV(GATE_BUTTON);
  } else {
    buttonState &= ~_BV(GATE_BUTTON);
  }

  if (pcf1.digitalRead(PUMP_BUTTON)) {
    buttonState |= _BV(PUMP_BUTTON);
  } else {
    buttonState &= ~_BV(PUMP_BUTTON);
  }

  if (pcf1.digitalRead(FAN_BUTTON)) {
    buttonState |= _BV(FAN_BUTTON);
  } else {
    buttonState &= ~_BV(FAN_BUTTON);
  }

  if (pcf1.digitalRead(FEEDER_BUTTON)) {
    buttonState |= _BV(FEEDER_BUTTON);
  } else {
    buttonState &= ~_BV(FEEDER_BUTTON);
  }
}
...
void check_relay_state_change (void) {
  if (actuator_state != lastActuatorState) {
    if ((actuator_state & _BV(GATE_RELAY)) != (lastActuatorState & _BV(GATE_RELAY))) {
      if (!(actuator_state & _BV(GATE_RELAY))) {
    ...
    if ((actuator_state & _BV(PUMP_RELAY)) != (lastActuatorState & _BV(PUMP_RELAY))) {
      if (!(actuator_state & _BV(PUMP_RELAY))) {
    ...
    if ((actuator_state & _BV(FAN_RELAY)) != (lastActuatorState & _BV(FAN_RELAY))) {
      if (!(actuator_state & _BV(FAN_RELAY))) {
    ...
    lastActuatorState = actuator_state;
  }
}
...
void check_serial_command (void) {
  while (Serial.available() > 0) {
    ...
    switch (cmd) {
      ...
      case 's':
      case 'S':
        Serial.println("== FEEDER SCHEDULE ==");
        for (int i = 0; i < 4; i++) {
          Serial.printf("#%d %s %s exec=%d\n",
            i+1,
            feederSchedule[i].time,
            feederSchedule[i].enabled ? "ON":"OFF",
            feederSchedule[i].executed
          );
        }

        Serial.println("== WATER SCHEDULE ==");
        for (int i = 0; i < 4; i++) {
          Serial.printf("#%d %s %s exec=%d\n",
            i+1,
            waterSchedule[i].time,
            waterSchedule[i].enabled ? "ON":"OFF",
            waterSchedule[i].executed
          );
        }
        return;
    }
...
void firebase_schedule_sync (void) {
  ...
  for (int i = 0; i < 4; i++) {
    String url = FIREBASE_URL + String("/schedules/feeder/") + String(i + 1) + ".json";

    if (!ensure_firebase_connection(url)) continue;
  ...
  for (int i = 0; i < 4; i++) {
    String url = FIREBASE_URL + String("/schedules/water/") + String(i + 1) + ".json";
...
void schedule_tick_task (void) {
  ...
  for (int i = 0; i < 4; i++) {
    /* For debug purposes */
    // Serial.printf("[DBG] now=%s slot=%s enabled=%d exec=%d\n",
    //   now.c_str(),
    //   feederSchedule[i].time,
    //   feederSchedule[i].enabled,
    //   feederSchedule[i].executed
    // );
    ...
    if (waterSchedule[i].enabled &&
        !waterSchedule[i].executed &&
        strcmp(now.c_str(), waterSchedule[i].time) == 0) {
      
      actuator_state &= ~_BV(PUMP_RELAY);
      Serial.printf("[SCHEDULE] Water slot %d triggered\n", i + 1);
      waterSchedule[i].executed = true;
    }
  }
}
